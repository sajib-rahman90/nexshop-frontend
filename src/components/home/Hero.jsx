...
const BADGES = [
  { icon: <Star size={12} className="fill-yellow-400 text-yellow-400" />, text: "4.9/5 from 48k+ reviews" },
  { icon: <ShieldCheck size={12} className="text-emerald-500" />, text: "Buyer Protected" },
  { icon: <Zap size={12} className="text-blue-500" />, text: "AI-Powered Discovery" },
];

export function Hero() {
  return (
    <section className="relative overflow-hidden pt-20 pb-24 lg:pt-32 lg:pb-36" style={{ background: "var(--background)" }}>
      {/* Background blobs */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl text-center">
        ...
        <motion.p
          ...
          style={{ color: "var(--foreground)" }}
        >
          AI-powered recommendations, real-time intelligence, and next-generation products — all in one place. Find exactly what you need, before you even know you need it.
        </motion.p>
        ...
        >
          {BADGES.map((badge) => (
            <div key={badge.text} className="flex items-center gap-1.5 text-xs font-medium opacity-60" style={{ color: "var(--foreground)" }}>
              {badge.icon}
              {badge.text}
            </div>
          ))}
        ...
        >
          <div className="flex justify-center gap-4 flex-wrap">
            {[
              { name: "NeuraWatch X Pro", price: "$359", tag: "AI Health", img: "https://images.unsplash.com/photo-1579586337278-3befd40fd17a?auto=format&fit=crop&q=80&w=300&h=300", discount: "-10%" },
              { name: "SonicAI Headphones", price: "$249", tag: "AI Audio", img: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&q=80&w=300&h=300", discount: null },
              { name: "VisionQuest VR 2", price: "$639", tag: "Virtual Reality", img: "https://images.unsplash.com/photo-1622979135225-d2ba269cf1ac?auto=format&fit=crop&q=80&w=300&h=300", discount: "-20%" },
            ].map((card, i) => (
              <motion.div
                key={card.name}
              ...
              >
                ...
                <div className="p-3">
                  <span className="text-[9px] font-bold uppercase tracking-widest text-blue-500">{card.tag}</span>
                  <p className="text-xs font-semibold mt-0.5 line-clamp-1" style={{ color: "var(--foreground)" }}>{card.name}</p>
                  <p className="text-sm font-black text-blue-500 mt-1">{card.price}</p>
                </div>
              </motion.div>
            ))}
          </div>
